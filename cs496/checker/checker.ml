...
and
  ...
  | Record(fs) -> RecType(List.map (fun(id,e) -> (id, eval_expr en e)) fs)
  | Proj(e,field_id) -> (*complete*)
    let te = type_of_expr en e
    in (match List.assoc_opt (type_of_expr tenv field_id) (type_of_expr tenv(recVal_to_list te) )with
      |None -> failwith "Field Type bad"
      |Some ev -> ev) (*also needs to check for duplicates of fields with List.mem*)
  | Debug ->

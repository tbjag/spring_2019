...
and
  ...
  | Let(x, e1, e2) ->
    ...
    in eval_expr (extend_env en x (RefVal (Store.new_ref g_store
                                               v))) e2
  ...
  | Set(var,exp1) ->
    let v = eval_expr en exp1
    in begin
         Store.set_ref g_store (refVal_to_int @@ from_some (apply_env g_store en var)) v;
         UnitVal
       end;
  | BeginEnd(es) -> eval_exprs en es
  | Debug ->
...
and
  eval_exprs en = function
  | [] -> UnitVal
  | [e] -> eval_expr en e
  | e::es ->
    let _ = eval_expr en e
    in eval_exprs en es
(*
